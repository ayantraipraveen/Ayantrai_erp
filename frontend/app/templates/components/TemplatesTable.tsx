        ...
        <table className="w-full text-left border-collapse table-fixed">
          ...
          <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-[#0e1219] border-b border-slate-200 dark:border-zinc-800/80 text-[10px] font-mono uppercase tracking-wider text-slate-500 dark:text-zinc-400 select-none">
            <tr>
              ...
              <th className="py-2 px-3 font-semibold w-[21%]">Configured Modules</th>
              <th className="py-2 px-3 font-semibold w-[11%]">Status</th>
            ...
            {paginatedTemplates.map((template) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-start gap-2 min-w-0">
                      ...
                      <div className="min-w-0 flex-1 overflow-hidden">
                        <div className="flex items-center gap-1 mb-0.5">
                          <span className="font-mono text-[9px] px-1 py-0.5 rounded bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 font-bold border border-slate-200 dark:border-zinc-700/60 flex-shrink-0">
                            {template.id}
                          </span>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {template.blocks.slice(0, 2).map((blk) => (
                        <Tooltip key={blk.id} content={blk.title} position="top">
                          <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-800/80 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-700/60 truncate max-w-[90px] cursor-default">
                            {blk.title}
                          </span>
                        </Tooltip>
                      ))}
                      {template.blocks.length > 2 && (
                        <Tooltip
                          ...
                          position="top"
                        >
                          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-700 dark:text-[#9D61FF] border border-purple-500/30 font-bold cursor-default">
                            +{template.blocks.length - 2}
                          </span>
                        </Tooltip>
                      )}
                  ...
                  <td className="py-2 px-3 overflow-hidden">
                    <span
                      className={`inline-flex items-center gap-1 text-[9px] font-mono px-1.5 py-0.5 rounded-full border uppercase font-bold ${
                        isActive
                    ...
                    {isRejected && template.rejection_reason && (
                      <Tooltip content={template.rejection_reason} position="bottom">
                        <div className="text-[9px] text-rose-500 dark:text-rose-400 mt-0.5 truncate cursor-default">
                      ...
                      {activeRole === "superadmin" && isPending && (
                        <Tooltip content="Review and approve" position="top">
                          <button
                            ...
                            className="h-6 px-1.5 rounded-md bg-[#9D61FF] hover:bg-[#8B4CF0] text-white font-bold text-[9px] flex items-center gap-0.5 transition-all cursor-pointer"
                          >
